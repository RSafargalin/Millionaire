...
public struct ITunesApp: Codable, ITunesEntityProtocol {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let dateString = try? container.decode(String.self, forKey: .currentVersionReleaseDate)
        let longDate = dateFormatter.date(from: dateString ?? "")
        dateFormatter.dateStyle = .medium
        dateFormatter.timeStyle = .medium
        dateFormatter.timeZone = .current
        let string = dateFormatter.string(from: longDate ?? Date())
        let date = dateFormatter.date(from: string)
        
        self.currentVersionReleaseDate = date
